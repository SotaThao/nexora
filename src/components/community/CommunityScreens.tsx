...
export function CommunityCreateWizard() {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (!name.trim() || isAnonymous) return; create.mutate({ name, slug: createCommunitySlug(name), description, kind, visibility }, { onSuccess: (community) => navigate(`/community/${community.id}`, { replace: true }) }) }
