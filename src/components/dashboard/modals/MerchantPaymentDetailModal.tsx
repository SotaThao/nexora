...
import {
  ...
  isDirectPaymentRecordCompleted,
...
export default function MerchantPaymentDetailModal({
  payment,
  isLoading = false,
  onClose,
  onAcknowledge,
  ...
  payment: MerchantPaymentRecord | null
  isLoading?: boolean
  onClose: () => void
        ...
        <div className="mb-4 flex items-center justify-between border-b border-nexoraBorder pb-4">
          <div>
            <span className="text-[10px] font-black uppercase tracking-wider text-nexoraMuted">
              {t('merchant_payments.detail_title')}
            </span>
            <p className="mt-0.5 text-xs text-nexoraMuted">{t('merchant_payments.detail_desc')}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          <div className="space-y-5">
            <div className="flex flex-col items-center justify-center rounded-xl border border-slate-100 bg-slate-50 py-4">
              ...
              <div className="mt-2 flex flex-col items-center gap-1">
                <DirectPaymentStatusBadge status={paymentStatus} t={t} size="md" className="mt-0" />
                {!completed ? (
                  <p className="max-w-xs text-center text-[11px] leading-relaxed text-nexoraMuted">
                    {t(getDirectPaymentStatusDescKey(paymentStatus))}
                  </p>
                ) : null}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-4 border-t border-nexoraBorder pt-4 text-xs">
              ...
              <div>
                <span className="block text-[10px] font-bold text-nexoraMuted">
                  {t('merchant_payments.col_method')}
                </span>
              ...
              {payment.customerConfirmedAt || payment.merchantConfirmedAt ? (
                <>
                  <div>
                    <span className="block text-[10px] font-bold text-nexoraMuted">
                      {t('merchant_payments.customer_confirmed_at')}
                    </span>
                    <span className="mt-0.5 block font-semibold text-nexoraText">
                  ...
                  <div>
                    <span className="block text-[10px] font-bold text-nexoraMuted">
                      {t('merchant_payments.merchant_confirmed_at')}
                    </span>
                    <span className="mt-0.5 block font-semibold text-nexoraText">
                      {payment.merchantConfirmedAt
                        ? formatTransactionDateTime(payment.merchantConfirmedAt, currentLanguage)
                        : '—'}
                    </span>
                  </div>
                </>
            ...
            {awaitingAck && onAcknowledge ? (
              <div className="space-y-3 rounded-xl border border-violet-100 bg-violet-50/50 p-4">
                <p className="text-[11px] leading-normal text-violet-700">
                  {t('merchant_payments.confirm_receipt_help')}
                </p>
                <button
                  type="button"
                ...
                >
                  ...
                  {t('merchant_payments.confirm_receipt')}
                </button>
              </div>
            ) : null}
          </div>
