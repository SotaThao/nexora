...
import { DirectPaymentStatusBadge } from '../direct-payments/DirectPaymentStatusBadge'
import { normalizePaymentStatusValue } from '../../../utils/directPaymentStatus'

export default function MerchantPaymentAckNoticeDialog({
  payment,
  pendingCount = 1,
  onViewDetail,
  onClose,
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-900/55 p-4 backdrop-blur-sm sm:items-center">
      <div
        role="dialog"
        aria-labelledby="payment-ack-notice-title"
      ...
      >
        ...
        <div className="flex items-start gap-3 pr-6">
          ...
          <div className="min-w-0">
            <h2 id="payment-ack-notice-title" className="text-base font-extrabold text-nexoraText">
              {t('merchant_payments.ack_notice_title')}
            </h2>
            <p className="mt-1 text-xs leading-relaxed text-nexoraMuted">
              {pendingCount > 1
                ? t('merchant_payments.ack_notice_desc_many', { count: pendingCount })
                : t('merchant_payments.ack_notice_desc')}
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          >
            {t('merchant_payments.view_detail')}
          </button>
        </div>
      </div>
    </div>
