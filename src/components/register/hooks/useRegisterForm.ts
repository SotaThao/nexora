...
export function useRegisterForm({
  ...
  resumeOtpVerification = false,
  autoSendVerificationOnResume = false,
  resumeEmail = "",
  resumePassword = "",
  resumeRole = null,
  initialRefCode = "",
  ...
  const [currentStep, setCurrentStep] = useState(
    resumeOtpVerification ? 2 : initialStep,
  );
  ...
  const handleAvatarFileChange = async (file: File) => {
    ...
    if (!AVATAR_ALLOWED_TYPES.includes(file.type)) {
      setErrors((prev) => ({
        ...prev,
        avatar: "errors.image_unsupported_file_type",
      }));
      return;
    }
    if (file.size > AVATAR_MAX_SIZE) {
      setErrors((prev) => ({
        ...prev,
        avatar: "errors.image_file_size_exceeded_5mb",
      }));
      return;
    }
  ...
  const handleSimulateVerify = () => {
    ...
    apiAuthAdapter
      .verifyEmail({ token: simToken, email: email.trim().toLowerCase() })
      .then(() => {
        ...
        return apiAuthAdapter.signInForInviteAccept({
  ...
  const handleResendVerification = () => {
    ...
    setResendMessage("");
    apiAuthAdapter
      .resendVerificationEmail({ email: email.trim().toLowerCase() })
      .then(() => {
  ...
  useEffect(() => {
    ...
    const resumeVerification = async () => {
      try {
        await apiAuthAdapter.resendVerificationEmail({
          email: resumeEmail.trim().toLowerCase(),
        });
        if (cancelled) return;
        setResendMessage(t("register.resend_verification_success"));
        setResendTimer(60);
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (!token) {
      setOtpError(
        t("components.register.hooks.useRegisterForm.invalidCodeTipEnter"),
      );
      return;
    }
    ...
    try {
      try {
        ...
        if (
          !isApiError(err) ||
          err.errorCode !== "USER_EMAIL_ALREADY_VERIFIED"
        ) {
          throw err;
        }
      }
  ...
  const savePayoutAccount = (e) => {
    ...
    if (!editValue.trim()) {
      setModalError(
        t("components.register.hooks.useRegisterForm.thisFieldIsRequired"),
      );
      return;
    }
    setPayouts((prev) => ({
  ...
  const handlePersonalRegisterSubmit = async () => {
    ...
    const hasConfiguredPayout = Object.values(payouts).some(
      (p) => p.enabled && p.value.trim(),
    );
    if (!hasConfiguredPayout) {
      setErrors({
        payout: t(
          "components.register.hooks.useRegisterForm.thisFieldIsRequired",
        ),
      });
      return;
    }
    ...
    try {
      ...
      const existingIdx = staffList.findIndex(
        (s) => s.id === staffId || s.email === email || s.phone === phone,
      );
      if (existingIdx !== -1) {
      ...
      saveMerchantSetupMutation
        .mutateAsync(parsedSetup)
        .catch((err) =>
          logger.error(
            "Failed to save merchant setup during registration",
            err,
          ),
        );
  ...
  const getStepName = (step) => {
    if (role === "business") {
      switch (step) {
        case 0:
          return t("components.register.hooks.useRegisterForm.accountType");
        case 1:
          return t("components.register.hooks.useRegisterForm.credentials");
        case 2:
          return t("components.register.hooks.useRegisterForm.activateOtp");
        default:
          return "";
      }
    } else {
      switch (step) {
        ...
        case 2:
          return t("components.register.hooks.useRegisterForm.activateOtp");
        case 3:
          return t("components.register.hooks.useRegisterForm.profileSetup");
        default:
          return "";
      }
    }
