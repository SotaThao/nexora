const nodes = ['48:1712', '48:20', '48:349', '48:536'].map(id => figma.getNodeById(id));
const audit = nodes.map(node => {
  if (!node) return { id: null, error: 'Node not found' };
  ...
  function traverse(n) {
    ...
    if (n.cornerRadius !== undefined && typeof n.cornerRadius === 'number' && n.cornerRadius > 0) {
      const bound = n.getConstrainedVariantProperties && n.getBoundVariableForProperty && n.getBoundVariableForProperty('topLeftRadius'); // check one corner
      if (!bound) {
    ...
    if (n.layoutMode && n.layoutMode !== 'NONE') {
      if (n.itemSpacing && typeof n.itemSpacing === 'number' && n.itemSpacing > 0) {
        const bound = n.getBoundVariableForPro
