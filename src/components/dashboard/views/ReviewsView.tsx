...
function ReviewsView({ reviews, reviewsPage, pageNumber, setPageNumber, staff, filter, setFilter, setupData, isLoading = false }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between bg-white p-3 rounded-xl border border-nexoraBorder/60 shadow-sm">
        ...
        <div className="space-y-1 w-full lg:w-auto">
          <span className="text-[10px] font-bold uppercase tracking-wider text-nexoraMuted block">
            {t('dashboard.review_kpi.review_source')}
          </span>
          <div className="flex flex-wrap gap-1.5">
            {[
              { id: 'all', label: `${t('staff_detail.tab_all')} (${counts.all})` },
              { id: 'google', label: `${t('dashboard.review_kpi.google_reviews')} (${counts.google})` },
              { id: 'yelp', label: `${t('dashboard.review_kpi.yelp_reviews')} (${counts.yelp})` },
              { id: 'low_stars', label: `${t('dashboard.review_kpi.low_stars_filter')} (${counts.lowStars})` }
            ].map((tab) => (
              <button
                key={tab.id}
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center shrink-0 w-full lg:w-auto">
          ...
          <div className="space-y-1 w-full sm:w-48">
            <span className="text-[10px] font-bold uppercase tracking-wider text-nexoraMuted block">
              {t('dashboard.review_kpi.star_rating')}
            </span>
            <CustomSelect
              size="sm"
              ...
              options={[
                { value: 'all', label: `${t('dashboard.review_kpi.all_stars')} (${counts.all})` },
                  ...
                  <div className="self-end sm:self-start shrink-0">
                    {isGoogle && (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-black uppercase text-nexoraText tracking-wider">{t('dashboard.review_kpi.google_reviews')}</span>
                    ...
                    {isYelp && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        title={t('common.view_on_yelp')}
                        className="inline-flex items-center gap-1.5 rounded-lg border border-nexoraBorder bg-white px-2.5 py-1.5 hover:bg-slate-50 transition shadow-sm hover:scale-[1.03] duration-150 cursor-pointer"
                      >
                        <svg viewBox="0 0 24 24" className="h-4 w-4 fill-walletYelp" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span className="text-[10px] font-black uppercase text-nexoraText tracking-wider">{t('dashboard.review_kpi.yelp_reviews')}</span>
                        <ExternalLink className="h-3 w-3 text-nexoraMuted" />
                      </a>
                    )}
                    {isPrivate && (
      ...
      {reviewsPage && reviewsPage.totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-nexoraBorder bg-white px-4 py-3 sm:px-6 mt-4 rounded-xl shadow-nexora-soft">
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-nexoraText">
                {t('common.showing') || 'Showing'}{' '}
                <span className="font-medium">{(pageNumber - 1) * 20 + 1}</span>{' '}
                {t('common.to') || 'to'}{' '}
                <span className="font-medium">
                  {Math.min(pageNumber * 20, reviewsPage.totalCount)}
                </span>{' '}
                {t('common.of') || 'of'}{' '}
                <span className="font-medium">{reviewsPage.totalCount}</span>{' '}
                {t('dashboard.menu.reviews') || 'results'}
              </p>
            </div>
            <div>
              <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                <button
                  ...
                  disabled={!reviewsPage.hasPreviousPage || isLoading}
                  className="relative inline-flex items-center rounded-l-md px-2 py-2 text-nexoraMuted ring-1 ring-inset ring-nexoraRule hover:bg-gray-50 focus:z-20 focus:outline-offset-0 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <span className="sr-only">Previous</span>
                  <ChevronLeft className="h-5 w-5" aria-hidden="true" />
                </button>
                <span className="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-nexoraText ring-1 ring-inset ring-nexoraRule">
                  {pageNumber} / {reviewsPage.totalPages}
                </span>
                <button
                  onClick={() => setPageNumber(Math.min(reviewsPage.totalPages, pageNumber + 1))}
                  disabled={!reviewsPage.hasNextPage || isLoading}
                  className="relative inline-flex items-center rounded-r-md px-2 py-2 text-nexoraMuted ring-1 ring-inset ring-nexoraRule hover:bg-gray-50 focus:z-20 focus:outline-offset-0 disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          <div className="flex flex-1 justify-between sm:hidden">
            <button
              onClick={() => setPageNumber(Math.max(1, pageNumber - 1))}
              disabled={!reviewsPage.hasPreviousPage || isLoading}
              className="relative inline-flex items-center rounded-md border border-nexoraRule bg-white px-4 py-2 text-sm font-medium text-nexoraText hover:bg-gray-50 disabled:opacity-50"
            >
              Previous
            </button>
            <span className="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-nexoraText">
              {pageNumber} / {reviewsPage.totalPages}
            </span>
            <button
              onClick={() => setPageNumber(Math.min(reviewsPage.totalPages, pageNumber + 1))}
              disabled={!reviewsPage.hasNextPage || isLoading}
              className="relative ml-3 inline-flex items-center rounded-md border border-nexoraRule bg-white px-4 py-2 text-sm font-medium text-nexoraText hover:bg-gray-50 disabled:opacity-50"
            >
              Next
            </button>
          </div>
