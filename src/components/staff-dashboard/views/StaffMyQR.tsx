...
export default function StaffMyQR() {
  ...
  const simulateMerchantScan = (businessNameInput) => {
    ...
    const timer = setTimeout(() => {
      ...
      saveMerchantSetupMutation.mutate({ ...merchantSetup, staffList: updatedStaffList } as any)
  ...
  const handleRequestUnlink = (businessName) => {
    ...
    try {
      if (merchantSetupData) {
        const updatedSetup = { ...(merchantSetupData as LooseObject) }
        if (Array.isArray(updatedSetup.staffList)) {
          updatedSetup.staffList = updatedSetup.staffList.filter((s: LooseObject) => s.id !== staffMember.id)
        }
        if (Array.isArray(updatedSetup.touchPoints)) {
          updatedSetup.touchPoints = updatedSetup.touchPoints.filter(
            (tp: LooseObject) => !(tp.type === 'Staff QR' && tp.staffId === staffMember.id)
          )
        }
