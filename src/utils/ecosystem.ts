...
export function resolveEcosystemBrandKey(name: string): string | null {
  ...
  if (normalized.includes('moonglemall') || normalized.includes('moongle')) return BRAND_NAME_ECO.moonglemall
  if (normalized.includes('cryptomap')) return BRAND_NAME_ECO.cryptomap360
  if (normalized.includes('vlinkpay') || normalized.includes('merchantportal')) return 'vlinkpay'
...
export function updateWindowUrl(targetWindow: Window | null, url: string): void {
