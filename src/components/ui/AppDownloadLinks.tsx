...
function GooglePlayLogo() {
  return (
    <svg className="h-4 w-4 shrink-0 sm:h-6 sm:w-6" viewBox="0 0 48 48" aria-hidden="true">
...
function AppStoreLogo() {
  return (
    <svg className="h-4 w-4 shrink-0 text-white sm:h-6 sm:w-6" viewBox="0 0 48 48" aria-hidden="true">
      <path
        fill="currentColor"
...
export default function AppDownloadLinks() {
  ...
  return (
    <div className="flex min-w-0 shrink items-center justify-end gap-2 sm:w-auto sm:flex-wrap sm:gap-3">
      {items.map(({ id, href, eyebrowKey, titleKey, Logo }) => (
        <a
          ...
          className="flex h-9 w-[6.25rem] items-center gap-1.5 rounded-md bg-black px-2 text-white shadow-sm ring-1 ring-black/10 transition hover:bg-slate-900 sm:h-10 sm:w-36 sm:gap-2 sm:px-3"
        >
          <Logo />
          <span className="min-w-0 leading-none">
            <span className="block truncate text-[5.5px] font-bold uppercase tracking-wide text-white/80 sm:text-[7px]">
              {t(eyebrowKey)}
            </span>
            <span className="mt-0.5 block truncate text-[10px] font-extrabold tracking-tight text-white sm:text-[15px]">
              {t(titleKey)}
            </span>
          </span>
        </a>
