...
function normalizeKybStatus(
  ...
  if (
    normalized === "verified_lite" ||
    normalized === "lite_pending" ||
    normalized === "pro_pending"
  ) {
    return normalized;
  }
...
function extractKybStatus(source: KybSource): string | null {
  ...
  const profileType = String(record.profileType ?? record.userType ?? "")
    .trim()
    .toLowerCase();
  ...
  for (const key of explicitKybKeys) {
    const status = normalizeKybStatus(record[key] as KybSource, {
      isExplicitKybField: true,
    });
...
export const apiAuthAdapter = {
  async login({
    email,
    password,
  }: LoginCredentials): Promise<AuthSession | null> {
