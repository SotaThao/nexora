...
import {
  usePendingAccounts,
  useReplaceAllPendingAccounts,
} from "../../../data/hooks/usePendingAccounts";
import {
  useAcceptStaffInvite,
  usePublicMerchantInvite,
  useStaffInviteInfo,
...
export default function useStaffRegistration({ inviteData }) {
  ...
  const apiInviteInfo =
    inviteInfoQuery.data ?? publicInviteInfoQuery.data ?? null;
  ...
  const [staffPaymentMethods, setStaffPaymentMethods] = useState<
    PaymentMethodDto[]
  >([]);
  ...
  useEffect(() => {
    ...
    if ((isApiInvite || isPublicInvite) && apiInviteInfo) {
      ...
      setNickname(
        apiInviteInfo.invitedName
          ? apiInviteInfo.invitedName.split(" ")[0] + "."
          : "",
      );
      setPosition(apiInviteInfo.invitedPosition || "Nail Technician");
      setRegReferralLink(
        formatReferralDisplay(
          apiInviteInfo.businessName,
          apiInviteInfo.refCode || inviteRefCode,
        ),
      );
    ...
    if (inviteData && !isApiInvite) {
      ...
      setRegReferralLink(
        formatReferralDisplay(
          inviteData.biz,
          inviteData.refCode || inviteData.referralCode,
        ),
      );
  ...
  useEffect(() => {
    ...
    staffPaymentMethodsRepository
      ...
      .catch((err: unknown) => {
        logger.warn(
          "Could not fetch payment methods for registration step",
          err,
        );
      });
  ...
  const handleSearchIdChange = (val) => {
    ...
    const timer = setTimeout(() => {
      ...
      if (profile) {
        ...
        setSearchError(
          t(
            "components.staff_registration.hooks.useStaffRegistration.nexoraStaffIdNot",
          ),
        );
      }
    }, 600);
  ...
  const getJoinRequestErrorMessage = (err: unknown) => {
    ...
    return t(
      "components.staff_registration.hooks.useStaffRegistration.joinRequestFailed",
      {
  ...
  const handleRegisterSubmit = (e) => {
    ...
    if (!regEmail.trim()) {
      errors.email = t(
        "components.staff_registration.hooks.useStaffRegistration.emailIsRequired",
      );
    } else if (!/\S+@\S+\.\S+/.test(regEmail)) {
      errors.email = t(
        "components.staff_registration.hooks.useStaffRegistration.emailIsInvalid",
      );
    }
    if (regEmail !== regConfirmEmail) {
      errors.confirmEmail = t(
        "components.staff_registration.hooks.useStaffRegistration.emailsDoNotMatch",
      );
    }
    if (!regPassword) {
      errors.password = t(
        "components.staff_registration.hooks.useStaffRegistration.passwordIsRequired",
      );
    } else if (regPassword.length < 6) {
      errors.password = t(
        "components.staff_registration.hooks.useStaffRegistration.passwordMustBeAt",
      );
    }
    if (!termsAccepted) {
      errors.terms = t(
        "components.staff_registration.hooks.useStaffRegistration.youMustAgreeTo",
      );
    }
    ...
    if (usesApiRegistration) {
      ...
      apiAuthAdapter
        ...
        .catch((err) => {
          ...
          if (
            isApiError(err) &&
            err.errorCode === "USER_EMAIL_ALREADY_EXISTS"
          ) {
            setRegErrors({
              email: t(
                "components.staff_registration.hooks.useStaffRegistration.emailAlreadyExists",
              ),
            });
          } else {
            showToast(
              currentLanguage === "vi"
                ? `Lỗi đăng ký: ${getApiErrorCode(err, "Vui lòng thử lại")}`
                : `Registration error: ${getApiErrorCode(err, "Please try again")}`,
              "error",
            );
          }
        })
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (otpCode.trim() === "1234") {
      ...
      setOtpError(
        t(
          "components.staff_registration.hooks.useStaffRegistration.invalidCodeTipEnter",
        ),
      );
    }
  };
  ...
  const savePayoutAccount = (e) => {
    ...
    if (!editValue.trim()) {
      setModalError(
        t(
          "components.staff_registration.hooks.useStaffRegistration.thisFieldIsRequired",
        ),
      );
      return;
    }
    ...
    if (isApiInvite) {
      ...
      try {
        ...
        if (!isStaffDisplayNameValid(finalDisplayName)) {
          const errorCode =
            getStaffDisplayNameErrorCode(finalDisplayName) ||
        ...
        showToast(
          currentLanguage === "vi"
            ? "Lỗi liên kết tiệm. Vui lòng thử lại."
            : "Failed to link with salon. Please try again.",
          "error",
        );
      } finally {
        setIsRegisterSubmitting(false);
    ...
    try {
      ...
      if (isExistingBusinessLinkError(err)) {
        try {
          ...
          logger.warn(
            "Could not refresh session for existing public join request",
            sessionErr,
          );
        }
      ...
      const errorMsg = isAlreadyLinked
        ? t(
            "components.staff_registration.hooks.useStaffRegistration.alreadyLinkedOrRequested",
          )
  ...
  const handleLinkLogin = async (e) => {
    ...
    if (!emailQuery) {
      setLinkError(
        t(
          "components.staff_registration.hooks.useStaffRegistration.emailIsRequired2",
        ),
      );
      return;
    }
    if (!passwordQuery) {
      setLinkError(
        t(
          "components.staff_registration.hooks.useStaffRegistration.passwordIsRequired2",
        ),
      );
      return;
    }
    ...
    try {
      ...
      const profileData = await httpClient.get<UserProfile>(
        "/api/v1/userprofile/me",
      );

      // Step 3: Only personal accounts can accept invite links
      const isMerchant =
        profileData.userType === "Merchant" ||
        profileData.profileType === "Merchant";
      if (isMerchant) {
      ...
      if (!hasOnboarded) {
        ...
        setPosition(apiInviteInfo?.invitedPosition || "Nail Technician");
        setStep(2);
        showToast(t("staff_registration.link.onboarding_required"), "info");
        return;
      }
      ...
      setLinkedProfile({
        fullName:
          profileData.fullName || profileData.email?.split("@")[0] || "",
        nickname:
      ...
      if (
        isApiError(err) &&
        (err.status === 401 ||
          err.errorCode === "USER_LOGIN_INVALID_USERNAME_OR_PASSWORD")
      ) {
        setLinkError(
          err.message || t("staff_registration.link.incorrect_password"),
        );
      } else {
  ...
  const handleLinkDecline = () => {
    ...
    showToast(
      t(
        "components.staff_registration.hooks.useStaffRegistration.linkRequestCancelled",
      ),
    );
  };
    ...
    if (usesApiRegistration) {
      ...
      try {
        if (isPublicInvite && pendingAvatarFile) {
          const uploaded =
            await imagesRepository.publicUpload(pendingAvatarFile);
          const uploadedUrl = uploaded.imageUrl || uploaded.fileUrl || "";
          if (!uploadedUrl) {
            showToast(t("errors.image_upload_failed"), "error");
            return;
        ...
        await apiAuthAdapter.signInForInviteAccept({
          email:
            regEmail || inviteData?.invitedEmail || inviteData?.email || "",
          password: regPassword,
        });
        ...
        try {
          ...
          logger.error(
            "Failed to persist personal onboarding profile",
            profileErr,
          );
        }
  ...
  const handleActivateProfile = async () => {
    ...
    const finalStaffMember = {
      ...
      flowType: isSelfServe
        ? "Self-Service Join"
        : inviteData?.isLinkOnly
          ? "Link Existing Staff ID"
          : "Invite New Staff",
    ...
    if (isApiInvite) {
      ...
      try {
        ...
        if (isLinkLoggedIn && needsOnboarding) {
          const refreshedProfile = await httpClient.get<UserProfile>(
            "/api/v1/userprofile/me",
          );
          setLinkedProfile((prev) => ({
            ...
            fullName:
              ...
              refreshedProfile.email?.split("@")[0] ||
              "",
            nickname:
              refreshedProfile.firstName ||
              refreshedProfile.email?.split("@")[0] ||
              "",
      ...
      try {
        ...
        if (isExistingBusinessLinkError(err)) {
          try {
            ...
            logger.warn(
              "Could not hydrate session or save payment methods for existing public join request",
              followUpErr,
            );
          }
          showToast(getPublicJoinAlreadyLinkedMessage(), "info");
    ...
    let parsedActive = merchantSetupQuery.data
    ...
    try {
      ...
      const existingIdx = staffList.findIndex(
        (s) =>
          s.id === inviteData?.id || s.email === email || s.phone === phone,
      );
      if (existingIdx !== -1) {
      ...
      saveMerchantSetupMutation
        .mutateAsync(parsedActive)
        .catch((err) =>
          logger.error(
            "Failed to save merchant setup during staff activation",
            err,
          ),
        );
      ...
      const newNoti = {
        ...
        type: "feedback_alert",
        title: t(
          "components.staff_registration.hooks.useStaffRegistration.newJoinRequest",
        ),
        message:
          currentLanguage === "vi"
        ...
        time: t(
          "components.staff_registration.hooks.useStaffRegistration.justNow",
        ),
        read: false,
        linkTab: "staff",
      };
      ...
      if (email.trim() && regPassword) {
        ...
        const filtered = existingAccounts.filter(
          (acc) => acc.email !== staffAccount.email,
        );
