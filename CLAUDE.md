...
1. **User story first.** Each story is one file in `user-story/` (`US-XXX-<slug>.md`, copy `user-story/_TEMPLATE.md`). If the user gives a raw requirement, draft the story (Story + AC + API Mapping + FE Surface) and get it approved before writing code. Status lifecycle: Draft → Grilled → Approved → Integrated → Tested → Done.
2. **Grill the story before approval.** Run the Grill Checklist in the template (use the `grill-me` skill if installed; otherwise apply the checklist manually): attack every AC for ambiguity, enumerate ALL states/branches (decline both sides, failure, duplicate, expiry, unlink), verify every endpoint against live Swagger or by a real probe request, and list what blocks implementation. A story with unanswered blocking questions stays Draft.
