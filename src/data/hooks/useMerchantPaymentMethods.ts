...
export function useSaveMerchantPayoutConfigs() {
  ...
  return useMutation({
    mutationFn: async (payoutConfigs: LooseObject) => {
      const methods = await merchantPaymentMethodsRepository.getAll()
      const tasks: Promise<void>[] = []

      for (const [uiKey, config] of Object.entries(payoutConfigs || {}) as [string, LooseObject][]) {
        const backendType = METHOD_TYPE_BY_UI_KEY[uiKey]
        if (!backendType) continue

        const method = (methods as LooseObject[]).find((m: LooseObject) => m.type === backendType)
        if (!method) continue
